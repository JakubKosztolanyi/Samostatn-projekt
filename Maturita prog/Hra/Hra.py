...
def spust_hru():
    ...
    # Herní informace
    info = {'score': 0, 'high_score': 0}

    # Panáček
    player_radius = 20
    player_x = window_x // 2
    player_y = window_y - 40 - player_radius
    player_speed_x = 0
    player_speed_y = 0
    max_speed_x = 5
    jump_speed = 15
    ...
    while running:
        ...
        last_platform_y = min([plat['rect'].y for plat in platforms]) if platforms else window_y
        ...
        if player_y > window_y:
            ...
            display.update()
            time.wait(2000)
            return  # vracíme se zpět do menu
...
# Pokud chceme spustit hru samostatně
